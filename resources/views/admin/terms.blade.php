    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('rules.store') }}" id="termsForm">
            ...
            <div class="mb-3">
                ...
                <textarea name="content_en" id="content_en" class="editor" style="height: 400px;">{{ old('content_en', $terms->content_en ?? '') }}</textarea>
            </div>
            ...
            @foreach ($languages as $code => $lang)
                <div class="mb-3">
                    ...
                    <textarea name="content_{{ $code }}" id="content_{{ $code == 'ua' ? 'uk' : $code }}" class="editor"
                        style="height: 400px;">{{ old("content_$code", $terms?->{"content_$code"} ?? '') }}</textarea>
                </div>
            @endforeach
